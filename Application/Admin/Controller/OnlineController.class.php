...
class OnlineController extends Controller {
    ...
    public function index(){
        ...
        $m->table('storage')->field('basic_goods.id as bbb ,storage_item.show_type as show_id,basic_goods.goods_basic_type,storage_item.id,goods_type.type_name as show_type,storage_item.online,storage_item.isnew,storage_item.price,storage_item.vip,storage_item.unit,storage_item.isnew,storage_item.buynum,storage.count,basic_goods.name,school.name as school_name')
            ...
            ->where($where)
            ->order('storage_item.oper_time desc');
        $m1 = clone($m);
        $m2 = clone($m);
...
//            echo $v['bbb'].',';
    ...
    public function toEdit(){
        ...
        $type_list = D('School')->getGoodsType($storage['school_id']);
