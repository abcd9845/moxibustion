...
$(function () {
  ...
  $('body').on('click', 'ul.acc-menu a', function() {
    ...
    console.log($(this).siblings('ul.acc-menu:visible'));
    if($(this).siblings('ul.acc-menu:visible').length>0) {
      $(this).closest('li').removeClass('open');
      $('ul.acc-menu', $(this).closest('li')).slideToggle();
    }
    else {
  ...
  $("a#leftmenu-trigger").click(function () {
      if ((window.innerWidth)<768) {
          ...
          if($.cookie('admin_leftbar_collapse') === 'collapse-leftbar') {
              $.cookie('admin_leftbar_collapse', '', {path:'/'});
              ...
              $('ul.acc-menu:first ul.acc-menu').css('visibility', 'hidden');
              $.cookie('admin_leftbar_collapse', 'collapse-leftbar', {path:'/'});
          }
      }
      checkpageheight();
      leftbarScrollShow();
  ...
  if($.cookie('full-narrow-page') === 'narrow-page') {
      $('#fullnarrowpage').toggles();
  } else {
      $('#fullnarrowpage').toggles({on:true});
  }

  $('.member-chooser-btn').click(function(){
...
function show_sky_pnotify(data) {
  $.pnotify({
    type: data.status ? 'success' : 'error',
    title: (typeof data.info === 'string') ? data.info : data.info.text,    
    right:0,
    icon:false,
    top:0,
    left:0
...
$('.check-all').click(function(){
  $('.ids:not(:disabled)', $(this).closest('table')).prop('checked', $(this).prop('checked'));
})